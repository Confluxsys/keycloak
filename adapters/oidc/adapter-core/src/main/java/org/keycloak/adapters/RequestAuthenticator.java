...
public abstract class RequestAuthenticator {
	...
	public String getTokenString(HttpFacade exchange) {
	...
	public AuthOutcome authenticate() {
		...
		log.infov("facade =>  {0}", facade);
		AuthOutcome outcome = bearer.authenticate(facade);
		log.infov("outcome =>  {0}", outcome.name());
		if (outcome == AuthOutcome.FAILED) {
			...
			if (verifySSL())
				return AuthOutcome.FAILED;
			String tokenStr = this.getTokenString(facade);
			try {
				Set<String> roles = ServerRequest.getRoles(deployment, tokenStr);
				log.infov("roles =>  {0}", roles);
				AdapterUtils.setRoles(roles);
			} catch (Exception e) {
				log.infov("Error occured while getting user info ",e);
			}
			completeAuthentication(bearer, "KEYCLOAK");
			log.infov("Bearer AUTHENTICATED");
		...
		log.infov(" query param auth outcome =>  {0}", outcome.name());
		if (outcome == AuthOutcome.FAILED) {
		...
		if (deployment.isEnableBasicAuth()) {
			...
			log.infov("basic auth outcome =>  {0}", outcome.name());
			if (outcome == AuthOutcome.FAILED) {
		...
		log.infov("OAuthRequestAuthenticator auth outcome =>  {0}", outcome.name());
		if (outcome == AuthOutcome.FAILED) {
		...
		AccessToken accessToken = oauth.getToken();
		
		AccessToken.Access access = accessToken.getRealmAccess();
		if (access != null) {
			log.infov("access token roles: {0} ",access.getRoles());
		}else {
			log.infov("access token does not have roles!!!. ");
			try {
				Set<String> roles = ServerRequest.getRoles(deployment, oauth.getTokenString());
				log.infov("roles =>  {0}", roles);
				AdapterUtils.setRoles(roles);
			} catch (Exception e) {
				log.infov("Error occured while getting user info ",e);
	...
	protected boolean verifySSL() {
		log.infov("facade.getRequest().isSecure(): {0} ", facade.getRequest().isSecure());

		log.infov("deployment.getSslRequired(): {0} ", deployment.getSslRequired());

		log.infov("facade.getRequest().getRemoteAddr(): {0} ", facade.getRequest().getRemoteAddr());
		if (!facade.getRequest().isSecure() && deployment.getSslRequired().isRequired(facade.getRequest().getRemoteAddr())) {
			log.infov("SSL is required to authenticate. Remote address %s is secure: %s, SSL required for: %s .", facade.getRequest().getRemoteAddr(),
					facade.getRequest().isSecure(), deployment.getSslRequired().name());
			return true;
		}
		return false;
	}
	...
	protected boolean isAutodetectedBearerOnly(HttpFacade.Request request) {
		if (!deployment.isAutodetectBearerOnly())
			return false;
	...
	protected void completeAuthentication(OAuthRequestAuthenticator oauth) {
		RefreshableKeycloakSecurityContext session = new RefreshableKeycloakSecurityContext(deployment, tokenStore, oauth.getTokenString(),
				oauth.getToken(), oauth.getIdTokenString(), oauth.getIdToken(), oauth.getRefreshToken());
		...
		log.infov("User ''{0}'' invoking ''{1}'' on client ''{2}''", principal.getName(), facade.getRequest().getURI(), deployment.getResourceName());
	}
	...
	protected void completeAuthentication(BearerTokenRequestAuthenticator bearer, String method) {
		...
		log.debugv("User ''{0}'' invoking ''{1}'' on client ''{2}''", principal.getName(), facade.getRequest().getURI(),
