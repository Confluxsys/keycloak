...
public class OAuthRequestAuthenticator {
	...
	protected String getRedirectUri(String state) {
		...
		log.debugf("scope query param: %s", scope);
		...
		KeycloakUriBuilder redirectUriBuilder = deployment.getAuthUrl().clone().queryParam(OAuth2Constants.RESPONSE_TYPE, OAuth2Constants.CODE)
				.queryParam(OAuth2Constants.CLIENT_ID, deployment.getResourceName())
				.queryParam(OAuth2Constants.REDIRECT_URI, rewrittenRedirectUri(url)).queryParam(OAuth2Constants.STATE, state)
				.queryParam("login", "true");
		
		log.debugf("redirectUriBuilder uri: %s", redirectUriBuilder);
		...
		scope = TokenUtil.attachOIDCScope(OAuth2Constants.SCOPE_PROFILE);
		log.debugf("attachOIDCScope: %s", scope);
	...
	protected AuthChallenge loginRedirect() {
		...
		return new AuthChallenge() {
			...
			public boolean challenge(HttpFacade exchange) {
				...
				exchange.getResponse().setCookie(deployment.getStateCookieName(), state, "/", null, -1,
						deployment.getSslRequired().isRequired(facade.getRequest().getRemoteAddr()), true);
	...
	protected AuthChallenge resolveCode(String code) {
		...
		try {
			...
			log.debug("reqAuthenticator.changeHttpSessionId(true):  " + reqAuthenticator.changeHttpSessionId(true));
			...
			log.error("failed to turn code into token: " + failure);
	...
	protected String stripOauthParametersFromRedirect() {
		KeycloakUriBuilder builder = KeycloakUriBuilder.fromUri(facade.getRequest().getURI()).replaceQueryParam(OAuth2Constants.CODE, null)
				.replaceQueryParam(OAuth2Constants.STATE, null).replaceQueryParam(OAuth2Constants.SESSION_STATE, null);
